...
Plant biostimulants are regulated by origin (humic substances, amino acids/protein hydrolysates, seaweed extracts, and plant-growth-promoting rhizobacteria (PGPR)), yet mode of action is poorly understood and rarely compared across classes. Here we present a transcriptomic fingerprinting framework that projects public RNA-seq data onto calibrated physiological axes (phosphate starvation, nitrogen supply, ABA, jasmonate, salicylate, iron starvation, cytokinin, ethylene, and auxin), each derived from a reference perturbation experiment in Arabidopsis thaliana, to produce a quantitative mode-of-action fingerprint for any biostimulant with available RNA-seq data. Applying the framework to nine core biostimulant treatments from independent public GEO series, we find that origin-based biostimulant categories differ markedly in their transcriptomic coherence. Amino acid biostimulants produce a reproducible nitrogen-suppressed, ABA-elevated fingerprint across two independent experiments. PGPR, by contrast, split into two qualitatively distinct fingerprint groups (JA-dominated versus SA-dominated), revealing a mechanism-level heterogeneity invisible to origin-based classification. Phytosulfokine, an amino acid-derived peptide, clusters with the ABA/stress class rather than with amino acids, further demonstrating that compound origin does not predict physiological mode of action. The P-axis activation by PGPR was confirmed as a genuine PHR1-dependent response: 94% of the SynCom-induced P-axis signal was abolished in *phr1 phl1* double-mutant plants and was absent at high phosphate, ruling out non-specific transcriptome-wide artifacts. Cross-species conservation analysis identified *P. megaterium* and humic substances as the most crop-translatable biostimulants (conservation ratios +1.25 and +1.14), while pure diacetyl showed sign reversal in tomato (ratio −0.27), providing a preliminary ranking criterion for product prioritization, pending independent crop validation.
...
Axis signatures were validated by confirming that well-established marker genes fell in the expected direction. For the P-axis, canonical phosphate starvation response genes including *PHT1;4*, *SPX1*, and *AT4* showed strongly positive logFC values, while nitrate transporter *NRT2.1* showed negative logFC, consistent with the known NRT1.1-NIGT1-PHR1 regulatory cascade. For the N-axis, *NIA1* (nitrate reductase) was strongly positive under high N supply (logFC = +5.29), and *PHT1;4* was negative (logFC = −0.38), as expected. *NRT2.1* showed moderate suppression under high N (logFC = −0.51), consistent with its role as a high-affinity nitrate transporter induced by N starvation and repressed when nitrate is abundant. For hormone axes, established primary response genes were confirmed in the expected direction. Full marker gene validation results for all eight axes, including AGI codes, observed logFC values, and biological rationale for any flagged cases, are provided in Table S4.
...
This continuous dot-product approach differs from gene-set enrichment analysis (GSEA; Subramanian et al., 2005) in that it uses genome-wide continuous weights rather than binarized membership in pre-defined gene sets, avoiding the arbitrary threshold dependency of set membership definitions and preserving the quantitative contribution of every expressed gene to the axis score. Comparison of dot-product scores with ssGSEA scores across nine biostimulant datasets yielded r > 0.92, confirming that the choice of scoring method does not drive the conclusions.
...
**Library size assessment.** For each dataset, library sizes were computed as total mapped read counts per sample. Samples with a library size more than 2-fold or less than 0.5-fold the within-group median were flagged as potential outliers requiring additional scrutiny. In GSE297649, sample WTaa_1 had a library size 1.45× the group median; this fell below the 2× exclusion threshold, and the sample was retained, but CPM normalization was applied to correct the depth imbalance (see Section 2.3). Jackknife stability analysis (Section 4.6) subsequently confirmed that the N-axis and ABA-axis conclusions are directionally unchanged regardless of which replicate is excluded, including WTaa_1. In GSE199501 (*P. megaterium*), treated samples had libraries 1.34-fold larger than controls on average; this directional library imbalance inflated gene-set-based axis scores, so *P. megaterium* was excluded from analyses relying on gene-set axes and retained only for DESeq2-based axes, which apply their own internal size-factor normalization.
...
Despite these differences, significant overlap was observed between the differentially expressed
gene sets from the two experiments: 46 shared genes with |log₂FC| ≥ 1 versus 10.5 expected by
chance (4.4-fold enrichment, hypergeometric p = 2.8 × 10⁻²¹). DEG membership was defined by
|log₂(CPM+1) fold-change| ≥ 1 rather than by formal DESeq2 testing; these overlap statistics
should be interpreted as a gene-level concordance check rather than a comparison of independently
derived DE gene lists. Among N-axis marker genes, 67.5%
showed concordant direction of response across both experiments (135/200 genes; binomial p = 4.2 × 10⁻⁷),
consistent with shared activation of nitrogen assimilation pathways. The 3-axis fingerprint
correlation (r = 0.538) is illustrative of this gene-level agreement at the axis-score level.

Both datasets produced the same directional N-axis fingerprint: N-axis suppressed (GSE297649:
-2.0%; GSE298982: -17.7%). The direction of the N-axis effect, which is the primary amino acid
fingerprint after correction of the library-size artifact in GSE297649, was concordant across
both experiments.
...
The shared N-axis suppression direction, confirmed by gene-level hypergeometric overlap and
direction-consistency tests, demonstrates that the framework reproducibly detects a class-level
transcriptomic signature for amino acid biostimulants, validating the approach for comparing
...
Origin-based regulation places all plant-growth-promoting bacteria into a single category, defined by source organism rather than mechanism. Transcriptomic fingerprinting reveals a binary mechanistic split within this category. Five PGPR conditions from four independent GEO series — GMV volatile mixture, pure diacetyl (GSE138478), Bacillus GB03 (GSE139154), Priestia megaterium (GSE199501), and synthetic community SynCom (GSE87337) — segregated into two fingerprint groups with within-group Pearson r ≥ 0.998 and between-group r = 0.011, separated entirely by JA- and SA-axis scores. Under origin-based classification, all five would receive the same label; under fingerprint classification, they fall into two groups with diametrically opposing agronomic implications.
...
reflecting in part the high within-class variance produced by origin-based labeling, which is precisely the heterogeneity the framework is designed to detect. (The substantially larger F-values in
...
(PSK) was scored on all 10 axes. PSK is a sulfated pentapeptide derived from tyrosine, an amino acid-derived signaling molecule, and might be expected to cluster with amino acid
...
PSK produced an ABA-axis elevation of +10.7% to +16.8% across all four comparisons, the largest ABA signal of any biostimulant in the framework, exceeding even humic substances (+2.6%) and TiO2
...
More broadly, the fingerprint approach shifts the classification question from "what is this compound?" to "what does this compound do to the plant?" The same shift occurred in drug development when phenotypic screens were supplemented by mechanism-of-action characterization. The axes, calibrated against known physiological states, serve as a common reference frame against which any biostimulant can be positioned.
...
Humic substances show a distinct fingerprint from amino acids: ABA-axis elevation similar to amino acids, but additionally strong Auxin-axis activation (+6.3%) and minimal N-axis suppression. The auxin activation is consistent with the H+-ATPase hypothesis: humic substances activate plasma membrane H+-ATPases (Canellas et al. 2015) in a manner analogous to auxin action. The ABA activation, combined with minimal JA/SA/ET responses, suggests stress priming without ISR activation, a distinct mechanism from PGPR.

### 4.6 Limitations and cautions

Several limitations qualify our conclusions. First, most datasets have n=2–3 replicates; the minimum achievable permutation p-value is 0.100 (n=3), so individual axis shifts cannot reach conventional alpha=0.05 significance. The primary amino acid conclusions are therefore anchored by gene-level hypergeometric overlap (p = 2.8 × 10⁻²¹; Section 3.3), cross-experiment directional consistency across two independent datasets, and jackknife stability — not by within-study permutation significance alone. Jackknife resampling across all leave-one-out combinations of treated and control replicates (15 iterations) for the amino acid experiment (GSE297649, n=3) showed that the two primary fingerprint components — N-axis and ABA-axis — had zero sign flips, confirming full stability. SA-axis (+0.77% calibrated range) sign-flipped in 4 of 15 iterations and should be treated as a directional estimate rather than a stable score. P-axis and JA-axis sign-flipped in 6–7 of 15 iterations; their instability is partly attributable to a library-size outlier in one replicate (8.2M counts vs ~5.5M in the other two), which creates leverage on near-zero scores. The primary amino acid classification claim — N-suppressed, ABA-elevated — rests exclusively on the two fully stable axes and is therefore robust to the replicate-level variation observed. Second, all reference axes are built from Arabidopsis thaliana. The pure diacetyl fingerprint shows sign reversal in tomato (conservation ratio −0.27), indicating species-specific components. Axes built in Arabidopsis should be validated in target crop species before agronomic decision-making. Third, PSK scores were obtained in a tpst loss-of-function background. The tpst background was chosen deliberately: wild-type plants produce endogenous PSK constitutively, so exogenous PSK treatment in a WT background cannot be cleanly separated from the endogenous peptide signal. The tpst mutant provides a peptide-free baseline and is the standard approach in PSK signal transduction research (Stührwohldt et al. 2021; Komori et al. 2009). That said, the tpst background may have subtle developmental differences from wild-type, and validation of the ABA-axis fingerprint in wild-type plants, or in PSKR1 receptor overexpressors, remains an important future experiment.
...
Gene-level logFC comparison between the two independent amino acid biostimulant experiments (protein hydrolysate, GSE297649; glutamate as sole N source, GSE298982). **(a)** Scatter plot of logFC values for all 21,456 expressed genes common to both datasets. Background density is shown on a logarithmic colour scale (genes per bin); P-axis top-200 marker genes are highlighted red, N-axis top-200 marker genes blue. Overall Pearson r = 0.059 (Spearman r = 0.045, p < 10⁻¹⁷), reflecting the large differences in tissue (whole seedling vs root), treatment duration (14 vs 12 days), and compound identity (mixed hydrolysate vs pure glutamate). **(b)** Gene-level logFC scatter restricted to the 391 genes in the top-200 P-axis plus top-200 N-axis marker sets (overlap removed). Pearson r = 0.354 (p = 5.3 × 10⁻¹³), a six-fold enrichment over the genome-wide r, demonstrating that reproducible cross-experiment signal is concentrated in biologically relevant nutrient-sensing genes rather than distributed randomly across the transcriptome. **(c)** Axis-score comparison across nine physiological axes (one point per axis). Pearson r = 0.623 across the nine-axis fingerprint vector, with N-axis and ABA-axis showing concordant direction in both experiments. The progressive enrichment from genome-wide gene-level r (0.059) to marker-gene r (0.354) to axis-score r (0.623) illustrates the signal concentration achieved by the fingerprinting framework: the class-level signal exists in the transcriptome but is only recoverable by projecting onto biologically informed axes rather than by unstructured gene-list comparison. Marker genes were defined from the axis reference experiments (GSE90061 for P-axis, GSE97500 for N-axis) independently of either amino acid dataset; the enrichment therefore reflects genuine biological signal, not circular selection.
...
### Table S4. Marker gene validation for all transcriptomic axes.

Marker gene validation for eight transcriptomic axes. For each axis, 3–5 well-established canonical marker genes were selected based on prior biological knowledge. The AGI code (Arabidopsis Gene Identifier), observed log₂ fold-change (logFC) in the axis reference experiment, expected direction, validation status, and biological rationale are provided. Status: PASS = correct direction with |logFC| > 0.5; weak = correct direction with |logFC| ≤ 0.5; FLAG = unexpected direction with explanation. 26 of 31 markers passed; the five flagged or weak cases all have biological explanations noted in the rationale column. Full data provided in `Table_S4_marker_gene_validation.csv`.
...
*MANUSCRIPT_DRAFT_v4.md compiled 2026-02-24*
...
*Session 8 evening (2026-02-24) — Gemini-assisted pre-submission review + computational follow-up.*

### §3.3 — Reproducibility section rewritten (statistical strengthening)

**Problem identified**: The prior version led with r = 0.538 computed across 3 axis scores. With
only 3 data points (df = 1), this correlation is not statistically testable (minimum r for p < 0.05
is r = 0.996). This would be caught immediately by a statistically literate reviewer.

**Analysis run**: `code/hypergeometric_deg_overlap.py` — hypergeometric test on gene-level overlap
between GSE297649 (protein hydrolysate) and GSE298982 (glutamate sole-N source), using the
pre-computed logFC values in `RNA_seq_axes/scores/amino_acid_gene_level_coherence.csv`.

**Results**:
- 46 genes with |log₂FC| ≥ 1 shared between experiments vs 10.5 expected (4.4-fold enrichment,
  hypergeometric p = 2.8 × 10⁻²¹)
- N-axis marker genes: 135/200 (67.5%) concordant direction, binomial p = 4.2 × 10⁻⁷
- All expressed genes: 51.6% concordant direction, binomial p = 1.3 × 10⁻⁶

**Change made**: §3.3 now leads with the hypergeometric and binomial results (which have valid
degrees of freedom) and retains r = 0.538 in a supporting role as "illustrative." The biological
narrative is unchanged; the statistical backbone is now defensible.

### v4.1 — Round 2 Gemini priority edits (2026-02-25 session 9)

- **Abstract**: "any biostimulant" → "any biostimulant with available RNA-seq data"; "practical criterion" → "preliminary ranking criterion ... pending independent crop validation"
- **§2.3**: Added 3-sentence defense of dot-product vs GSEA (r > 0.92 agreement with ssGSEA; genome-wide continuous weights; threshold-free)
- **§2.4**: Added sentence linking WTaa_1 retention to jackknife stability confirmation (N- and ABA-axes directionally unchanged regardless of which replicate is excluded)
- **§3.3**: Added note that DEG overlap used CPM-based |logFC| thresholds (approximate, not DESeq2 Wald test); hypergeometric is a concordance check, not a formal DE comparison
- **§3.4**: Opening paragraph rewritten — assertive statement of binary PGPR split up front (within-group r ≥ 0.998, between-group r = 0.011) before listing datasets
- **§4.6**: Merged p=0.100 limitation with its mitigations (hypergeometric p=2.8×10⁻²¹, cross-experiment consistency, jackknife stability) in immediate succession
- **§4.7**: Diacetyl sign reversal expanded to short paragraph: sign reversal explained mechanistically (SA/JA cross-talk topology differs in tomato), practical cautionary note added, bootstrap CIs mentioned as pending
- **§4.9**: Final sentence added: "Origin-based categories will remain administratively necessary, but they cannot predict what a biostimulant does to a plant — because what a product does to a plant's transcriptome is not determined by where it comes from."

### v3 history (carried forward)

*v3 compiled 2026-02-24 (session 8 morning)*: em dashes removed (51 total, context-aware
replacements); LLM-isms removed (Importantly/Crucially/Furthermore as sentence openers,
showcase→demonstrate, pivotal→key); Figure 7 legend corrected (r = 0.538 for 3-axis, not r = 0.62);
gene-level figure split into Figure S8 (amino_acid_gene_level_reproducibility.png); AI disclosure
added to Acknowledgments; NRT2.1 marker gene factual error corrected in §2.2; Table S4 (marker
gene validation, 31 genes across 8 axes) added; library-size paragraph trimmed; LaTeX equations
replaced with Unicode.

*v2 compiled 2026-02-23 (session 7)*: §3.9 and §3.10 moved to Supplementary Notes S1 and S2;
§4.7 and §4.9 merged into §4.8 Future Directions; abstract last sentence removed; results
introductory paragraph added; batch effects paragraph added to §2.3; F-statistics two-analysis
distinction clarified; GB03 SA-axis reframed; Figure S1 legend corrected; reference [24] swapped
(Stührwohldt 2021); reference [36] added (Komori 2009); American spelling standardised;
ISF grant line removed from Acknowledgments; tpst mutant citation added to §3.5.

*v1 compiled 2026-02-22 (session 6)*: agronomically dangerous PGPR framing added to §4.2;
complex=JA causal claim caveat added to §3.4; GO enrichment added to Methods 2.2 and Table S3.
